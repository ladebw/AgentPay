...
class TestAgentRoutes:
    ...
    async def test_get_agent_success(self, client):
        ...
        with patch.object(AgentService, 'get', new_callable=AsyncMock) as mock_get:
            ...
            response = await client.get("/api/v1/agents/test-id-123")
            
            assert response.status_code == 200
            data = response.json()
            assert data["id"] == "test-id-123"
            assert data["name"] == "Test Agent"
            mock_get.assert_called_once_with("test-id-123")
    ...
    async def test_update_agent_permissions_success(self, client):
        ...
        with patch.object(AgentService, 'get', new_callable=AsyncMock) as mock_get, \
             patch.object(AgentService, 'update_permissions', new_callable=AsyncMock) as mock_update:
            mock_get.return_value = mock_agent
            mock_update.return_value = mock_agent
            
            response = await client.put(
                "/api/v1/agents/test-id-123",
                json={
                    "permissions": {"allow": ["payment"]}
                }
            )
            ...
            assert data["id"] == "test-id-123"
            mock_get.assert_called_once_with("test-id-123")
            mock_update.assert_called_once_with("test-id-123", {"allow": ["payment"]})