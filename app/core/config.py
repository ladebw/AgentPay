...
class Settings(BaseSettings):
    ...
    webhook_signing_secret: str = Field(default="change-me", env="WEBHOOK_SIGNING_SECRET")
    ...
    @field_validator('api_key_salt', 'secret_key', 'webhook_signing_secret', mode='after')
    ...
    def validate_key_management_mode(cls, v, info):
        ...
        if environment == 'production':
            ...
            if v == KeyManagementMode.NON_CUSTODIAL and not info.data.get('private_key'):
                raise ValueError('private_key must be set when using NON_CUSTODIAL mode')
        return v
