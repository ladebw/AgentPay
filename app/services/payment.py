...
class PaymentService:
    ...
    async def process_payment(self, agent_id: str, invoice_id: str, amount: float) -> Payment:
        ...
        try:
            ...
            if self.redis_client:
                await asyncio.to_thread(self.redis_client.setex, idempotency_key, settings.idempotency_key_ttl, "processed")
    ...
    async def _send_transaction(self, payment: Payment) -> str:
        ...
        tx_result = await self.blockchain.transfer_usdc(
            ...
            key_manager=self.blockchain.key_manager if hasattr(self.blockchain, 'key_manager') and self.blockchain.key_manager else None
        )
